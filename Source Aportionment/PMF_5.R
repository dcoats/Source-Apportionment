...
##sname<-unlist(strsplit("default_gkey_6source.ini",".",fixed=TRUE))[1]
##sname
#####################################################################################
###################### PMF applied to current project with LA data ##################
#####################################################################################
...
VCt<-t(VC)
ysimst<-t(ysims)
write.table(ysimst,file="ysim500dat.txt",quote=F,row.names=FALSE,col.names=FALSE)
write.table(VCt,file="yVC500dat.txt",quote=F,row.names=FALSE,col.names=FALSE)
...
gkeyprep(yfleet1profiles,uncertainty,"yVC500dat.txt","ysim500dat.txt")     #gets ready to gkey
...
fac1<-fac
#fac1[,1]<-fac[,3]/83.59759
#fac1[,2]<-fac[,1]/135.25885
#fac1[,3]<-fac[,2]/932.02049
            ###for cv=.5
