...
source("profiles5low.R")  ### runs code to create single profile and VA
#source("profiles5high.R")   ###Change from low to high volatility for the profiles
...
datagen<-function(cv,cvlog)
{
  ...
  for(k in 1:120)
  {
    ...
    gasVA[k]<-((gasdir[1,1]^2)*gasunc[k,1])+((gasdir[1,2]^2)*gasunc[k,2])+((gasdir[1,3]^2)*gasunc[k,3])+((gasdir[1,4]^2)*gasunc[k,4])
  }
