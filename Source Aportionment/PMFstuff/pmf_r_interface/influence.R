...
#lam <- lamUNWT
#fac <- facUNWT
#lam <- lamfinal.0000001
#fac <- facfinal.0000001
#lam <- lamfinal100000
#fac <- facfinal100000
#lam <- lammet
#fac <- facmet
...
## not working: system(mypaste("copy"," gkeyn6ADJ.jpg"," gkeyn6UNWT.jpg"))
 ...
 lamplotsX11 <- function(lam,name="gkey"){
...
#   mypaste("testlam",i,".txt"),
#   mypaste("testq",i,".txt"),fpeak= 0.1,i,nrow(morgc),ncol(morgc))    
runpmfplain("default_nogkeyUNWT.ini","morgc.txt","morgu.txt",mypaste("testfac",i,".txt"),
   mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0.2,i,nrow(morgc),ncol(morgc))    
runpmfplain("default_nogkeyUNWTresids.ini","morgc.txt","morgu.txt",mypaste("testfac",i,".txt"),
   mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0.2,i,nrow(morgc),ncol(morgc))    

resids <- read.table("RESIDS.txt")
sum(resids^2)

lam<-read.table(mypaste("testlam",i,".txt"))
fac<-read.table(mypaste("testfac",i,".txt"))
plot(c(as.matrix(lam)),c(as.matrix(lamUNWT)))
...
sum(resids[,grp1]^2)                 # 13177.72
sum(resids[,grp2]^2)                 # 21469.66
sum(resids[,bothgroups]^2)           #  2361.104

sum(resids[,c(grp1,bothgroups)]^2)   # 15538.82
sum(resids[,c(grp2,bothgroups)]^2)   # 23830.77

#                    Q overall is      37008.48
#                    Q met is          13286.2246
#                    Q org is           5508.9224



## Evaluating starting values for referee
runpmfplainstartval <- function(inifilename,data,prec,F,Lambda,Q,fpeak=0,startval=2,sources=6,rows=nrow(data),cols=ncol(data))
{
...
inifile[17]<-paste("                           ",startval,"         0",sep="")
...
jj <- 1
lamstartval <- array(NA,dim=c(82,7,20))
facstartval <- array(NA,dim=c(102,7,20))

for (jj in 4:4)
{
 runpmfplainstartval("default_nogkeyUNWTresids.ini","morgc.txt","morgu.txt",mypaste("testfac",i,".txt"),
   mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0.0,startval=jj,i,nrow(morgc),ncol(morgc))    
         
 lamstartval[,,jj] <- as.matrix(read.table(mypaste("testlam",i,".txt")))
 facstartval[,,jj] <- as.matrix(read.table(mypaste("testfac",i,".txt")))
}

#faccorrstartval <- array(NA,dim=c(20,20,6))
faccorrstartval <- matrix(NA,20,20)
#for (ii in 2:20) for (jj in 1:(ii-1)) faccorrstartval[ii,jj] <- mean(diag(cor(facstartval[,,ii],facstartval[,,jj])))
for (ii in 2:20) for (jj in 1:(ii-1)) faccorrstartval[ii,jj] <- bestcor(facstartval[,,ii],facstartval[,,jj])

summary(faccorrstartval[lower.tri(faccorrstartval)])
boxplot(faccorrstartval[lower.tri(faccorrstartval)])

apply(faccorrstartval[-4,-4][-1,],1,min,na.rm=T) # 4th estimate a problem
   # rerun with new FPEAK

for (ii in 1:20) faccorrstartval[ii,4] <- bestcor(facstartval[,,ii],facstartval[,,4])
faccorrstartval[4,] <- faccorrstartval[,4]
...
## Try with just metalsplus                   Note: Q was 13286.2246
...
### NOTE July 5 2008: Q is 13286.2246
sum( (morgc[,1:44] - as.matrix(factemp) %*% t(as.matrix(lamtemp)))^2 / morgu[,1:44]^2 )
...
temp <-  apply(abs(dffitvals),2,median)
dfplot(dffitvals)
names[43:44] <- c("SO4","NO3")
	dfplotfigure <- function(dffitvals,namespec=names,ylabstring="D_k")
	{
		par(cex=1)
      	temp <- (apply(abs(dffitvals),2,median))
		plot(temp,ylim=c(0,max(temp)+.05),ylab="",
			xlab="",xaxt="n",type="n")
            mtext("Species",side=1,line=3.5)
		mtext(ylabstring,side=2,line=2.75,font=3,cex=1.25)
            par(cex=.6)
		axis(1,at=1:ncol(dffitvals),label=namespec,las=2)
		###  makes a box around significant values
		par(cex=1)
		for(ii in 1:length(temp)){
		  if(temp[ii]>=quantile(temp,.9,na.rm=TRUE)){
		  rect(ii-.5,par("usr")[3]+.05,ii+.5,par("usr")[4]-.05,col="grey90",border="transparent")
		}}
		points(temp,bty="o") #redraws the points on top of the shaded boxes
		#end of box maxing
	}
par(oma=c(0.5,.35,0,0))
#dfplotfigure(dffitvals,ylabstring=expression(tilde(D)[j]))

## FIGURE 2 in INFLUENCE PAPER
dfplotfigurelog <-function(dffitvals,namespec=names,ylabstring="D_k",
    lowercut=-3)
{
  par(cex=1)
      temp <- (apply(abs(dffitvals),2,median))
  plot(log(temp),ylim=c(lowercut,max(log(temp))+.05),ylab="",
      xlab="",xaxt="n",type="n")
            mtext("Species",side=1,line=3.5)
mtext(ylabstring,side=2,line=2.75,font=3,cex=1.25)
            par(cex=.6)
...
points(log(temp),bty="o") #redraws the points on top of the shaded boxes
#points((1:82)[log(temp) < lowercut],rep(lowercut,82)[log(temp) < lowercut],pch="*")
#end of box maxing
}

#dfplotfigure(dffitvals,ylabstring=expression(tilde(D)))
#mtext("j",side=2,line=2.25,adj=.515,cex=.71)

pdf("influence82log.pdf",width=11*1,height=8.5*1)
dfplotfigurelog(dffitvals,ylabstring=expression(log(tilde(D))))
mtext("j",side=2,line=2.25,adj=.54,cex=.71)
dev.off()
...
Ktemp[1,1] <- Knat <- round(sum(tempold[grp2])/sum(tempold[grp1]),3)   ## Grp2 inf / Grp 1 inf
...
#AAEREL with facmet .........BAD IDEA SINCE TOTAL MASS OF ORGANICS DIFFERS GREATLY WITH MASS OF METALS
AAERELwithfacmet <- c(
1,
bestaaeRELmat(facfinal.0000001,facmet),
bestaaeRELmat(facfinal.01,facmet),
bestaaeRELmat(facUNWT,facmet),  # unwt yields influence of 0.0358
bestaaeRELmat(facfinal.1,facmet),
bestaaeRELmat(facfinal1,facmet),
bestaaeRELmat(facfinal10,facmet),
bestaaeRELmat(facfinal100,facmet),
bestaaeRELmat(facfinal100000,facmet),
bestaaeRELmat(facorg,facmet))
...
bestcormatcors2 <- function(x,y)
{
  ...
  for (i in 1:length(perms)) meancor[i] <- mean(diag(mat[1:nrow(mat),perms[[i]]]))
  #max(meancor)
  return( diag(mat[1:nrow(mat),perms[[(1:length(perms))[meancor==max(meancor)]]] ]) ,
          perms[[(1:length(perms))[meancor==max(meancor)]]] ,
          mat )
}

bestcormatcors2(facfinal.0000001,facUNWT)
bestcormatcors2(facfinal100000,facUNWT)
bestcormatcors2(facmet,facUNWT)
bestcormatcors2(facorg,facUNWT)
bestcormatcors2(facfinal.0000001,facmet)
bestcormatcors2(facfinal100000,facorg)

bestcormatcors2(facUNWT,facmet)
bestcormatcors2(facUNWT,facorg)
bestcormatcors2(facfinal.0000001,facmet)
bestcormatcors2(facfinal.0000001,facorg)
bestcormatcors2(facfinal.01,facmet)
bestcormatcors2(facfinal.01,facorg)
bestcormatcors2(facfinal.1,facmet)
bestcormatcors2(facfinal.1,facorg)
bestcormatcors2(facfinal1,facmet)
bestcormatcors2(facfinal1,facorg)
bestcormatcors2(facfinal10,facmet)
bestcormatcors2(facfinal10,facorg)
bestcormatcors2(facfinal100,facmet)
bestcormatcors2(facfinal100,facorg)
bestcormatcors2(facfinal100000,facmet)
bestcormatcors2(facfinal100000,facorg)

bestcormatcors2(facmet,facorg)
cor(facmet)
cor(facorg)

#AAEREL with facUNWT.........BAD IDEA SINCE TOTAL MASS OF ORGANICS DIFFERS GREATLY WITH MASS OF METALS
AAERELwithfacUNWT <- c(
bestaaeRELmat(facmet,facUNWT),
bestaaeRELmat(facfinal.0000001,facUNWT),
bestaaeRELmat(facfinal.01,facUNWT),
bestaaeRELmat(facUNWT,facUNWT),  # unwt yields influence of 0.0358
bestaaeRELmat(facfinal.1,facUNWT),
bestaaeRELmat(facfinal1,facUNWT),
bestaaeRELmat(facfinal10,facUNWT),
bestaaeRELmat(facfinal100,facUNWT),
bestaaeRELmat(facfinal100000,facUNWT),
bestaaeRELmat(facorg,facUNWT))
...
#AAEREL with facorg.........BAD IDEA SINCE TOTAL MASS OF ORGANICS DIFFERS GREATLY WITH MASS OF METALS
AAERELwithfacorg <- c(
bestaaeRELmat(facmet,facorg),
bestaaeRELmat(facfinal.0000001,facorg),
bestaaeRELmat(facfinal.01,facorg),
bestaaeRELmat(facUNWT,facorg),  # unwt yields influence of 0.0358
bestaaeRELmat(facfinal.1,facorg),
bestaaeRELmat(facfinal1,facorg),
bestaaeRELmat(facfinal10,facorg),
bestaaeRELmat(facfinal100,facorg),
bestaaeRELmat(facfinal100000,facorg),
bestaaeRELmat(facorg,facorg))
...
plot(Ks,corrswithfacUNWT,ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,corrswithfacmet,col="blue",lty="dashed")
lines(Ks,corrswithfacorg,col="red",lty="dotted")
...
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   legend=c("Unmanipulated complete data","Metals data","Organics data")   ,
   lty=c("solid","dashed","dotted"),lwd=c(1,1,1),col=c("black","blue","red"))  #This creates the legend

#### FIGURE 6 (WITHOUT MET AND ORG) #####
par(mar=c(5,4,4,2)+.1)
par(oma=c(1,0,0,0))
Ks <- c(.0000001, .01, .0358, .1, 1, 10, 100, 100000)
plot(Ks,corrswithfacUNWT[-c(1,10)],ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,corrswithfacmet[-c(1,10)],col="blue",lty="dashed")
lines(Ks,corrswithfacorg[-c(1,10)],col="red",lty="dotted")
axis(1,at=Ks,labels=c(Ks),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   legend=c("Unmanipulated complete data","Metals data","Organics data")   ,
   lty=c("solid","dashed","dotted"),lwd=c(1,1,1),col=c("black","blue","red"))  #This creates the legend
#abline(h=corrswithfacmet[10],lty="dotted",lwd=.5,col="gray")
...
#### FIGURE For CHEMOMETRICS -- Compare3Contributions #####
pdf(file="Compare3Contributions.pdf",width=11*.8,height=8.55*.8)
par(mar=c(5,4,2,2)+.1)
par(oma=c(1,0,0,0))
Ks <- c(.0000001, .01, .0358, .1, 1, 10, 100, 100000)
plot(Ks,corrswithfacUNWT[-c(1,10)],ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,corrswithfacmet[-c(1,10)],col="black",lty="dashed",lwd=2)
lines(Ks,corrswithfacorg[-c(1,10)],col="black",lty="dotted",lwd=2)
axis(1,at=Ks,labels=c(Ks),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   legend=c("Unmanipulated complete data","Elements data","Organics data")   ,
   lty=c("solid","dashed","dotted"),lwd=c(1,2,2),col=c("black","black","black"))  #This creates the legend
#abline(h=corrswithfacmet[10],lty="dotted",lwd=.5,col="gray")
dev.off()

 
pdf(file="Compare3Profiles.pdf",width=11*.8,height=8.5*.8)
par(mar=c(5,4,2,2)+.1)
par(oma=c(1,0,0,0))
Ks <- c(.00000001,.0000001, .01, .0358, .1, 1, 10, 100, 100000, 1000000)
plot(Ks,expcorrswithlamUNWT,ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,expcorrswithlammet,col="black",lty="dashed",lwd=2)
lines(Ks,expcorrswithlamorg,col="black",lty="dotted",lwd=2)
axis(1,at=Ks,labels=c("MET",Ks[2:(length(Ks)-1)],"ORG"),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   legend=c("Unmanipulated complete data","Elements data","Organics data")   ,
   lty=c("solid","dashed","dotted"),lwd=c(1,2,2),col=c("black","black","black"))  
...
plot(Ks,expcorrswithlamUNWT,ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,expcorrswithlammet,col="blue",lty="dashed")
lines(Ks,expcorrswithlamorg,col="red",lty="dotted")
axis(1,at=Ks,labels=c("MET",Ks[2:(length(Ks)-1)],"ORG"),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
...
#### FIGURE 7 WITHOUT MET and ORG#####
#par(mar=c(5,4,4,2)+.1)
#par(oma=c(1,0,0,0))
Ks <- c(.0000001, .01, .0358, .1, 1, 10, 100, 100000)
plot(Ks,expcorrswithlamUNWT[-c(1,10)],ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,expcorrswithlammet[-c(1,10)],col="blue",lty="dashed")
lines(Ks,expcorrswithlamorg[-c(1,10)],col="red",lty="dotted")
axis(1,at=Ks,labels=c(Ks),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
...
facjjarrayADJUST <- facjjarray
for (ii in 1:44) for (source in 1:7) 
   facjjarrayADJUST[,source,ii] <- facjjarray[,source,ii] / (1 - lammetall[ii,source])

facjjaae <- rep(NA,44)
facjjcorr <- rep(NA,44)
facjjaaeADJUST <- rep(NA,44)
facjjcorrADJUST <- rep(NA,44)
facjjaaeREL <- rep(NA,44)
facjjaaeRELADJUST <- rep(NA,44)
for (ii in 1:44) 
{
  facjjaae[ii] <- bestaaemat(facjjarray[,,ii],facmetall)
  facjjaaeADJUST[ii] <- bestaaemat(facjjarrayADJUST[,,ii],facmetall)
  facjjaaeREL[ii] <- bestaaeRELmat(facjjarray[,,ii],facmetall)
  facjjaaeRELADJUST[ii] <- bestaaeRELmat(facjjarrayADJUST[,,ii],facmetall)
  facjjcorr[ii] <- bestcormat(facjjarray[,,ii],facmetall)
  facjjcorrADJUST[ii] <- bestcormat(facjjarrayADJUST[,,ii],facmetall)
}

infspec <- (apply(abs(dffitvals),2,median))
#plot(infspec)

plot(1-facjjcorr,type="l",ylim=c(0,1))
lines(1-facjjcorrADJUST,type="l",col="red")
lines(infspec/max(infspec),col="blue")

plot(facjjaae/max(facjjaae),type="l",ylim=c(0,1))
lines(facjjaaeADJUST/max(facjjaaeADJUST),type="l",col="red")
lines(infspec/max(infspec),col="blue")

plot(1-facjjcorr,infspec)
plot(facjjaae,infspec)
plot(facjjaaeADJUST,infspec)

cor(1-facjjcorr,infspec)
cor(facjjaae,infspec)
cor(facjjaaeADJUST,infspec)

plot(rank(1-facjjcorr),rank(infspec))
plot(rank(facjjaae),rank(infspec))
plot(rank(facjjaaeREL),rank(infspec))
plot(rank(facjjaaeADJUST),rank(infspec))
plot(rank(facjjaaeRELADJUST),rank(infspec))

plot((1-facjjcorr),(infspec))
plot((facjjaae),(infspec))
plot((facjjaaeREL),(infspec))
plot((facjjaaeADJUST),(infspec))
plot((facjjaaeRELADJUST),(infspec))
plot((diag(Hmat)),(infspec))

cor(rank(1-facjjcorr),rank(infspec))
cor(rank(facjjaae),rank(infspec))
plot(rank(facjjaaeREL),rank(infspec))
cor(rank(facjjaaeREL),rank(infspec))
cor(rank(facjjaaeADJUST),rank(infspec))
cor(rank(facjjaaeRELADJUST),rank(infspec))
cor(rank(diag(Hmat)),rank(infspec))
plot(rank(diag(Hmat2)),rank(infspec))
cor(rank(diag(Hmat2)),rank(infspec))

cor((facjjaaeREL),(infspec))
cor((diag(Hmat2)),(infspec))
...
cor((facjjaaeREL),(infspec))
cor((facjjaaeADJUST),(infspec))
cor((facjjaaeRELADJUST),(infspec))
cor((diag(Hmat)),(infspec))

cor(diag(Hmat),facjjaaeREL)
cor(rank(diag(Hmat)),rank(facjjaaeREL))


plot(rank(1-facjjcorr),type="b")
lines(rank(infspec),col="blue",type="b")
abline(v=(1:44),lty=3,col="lightgray")

plot(rank(facjjaaeREL),type="b")
lines(rank(infspec),col="blue",type="b")
abline(v=(1:44),lty=3,col="lightgray")

plot(rank(1-facjjcorr)[order(rank(1-facjjcorr))],type="p",xaxt="n")
axis(1,labels=metnames[order(rank(1-facjjcorr))],at=1:44,cex.axis=.4)
lines(rank(infspec)[order(rank(1-facjjcorr))],col="blue",type="b")

plot(rank(facjjaaeREL)[order(rank(facjjaaeREL))],type="p",xaxt="n")
axis(1,labels=metnames[order(rank(facjjaaeREL))],at=1:44,cex.axis=.4)
lines(rank(infspec)[order(rank(facjjaaeREL))],col="blue",type="b")

plot(rank(facjjaae)[order(rank(facjjaae))],type="p",xaxt="n")
axis(1,labels=metnames[order(rank(facjjaae))],at=1:44,cex.axis=.4)
lines(rank(infspec)[order(rank(facjjaae))],col="blue",type="b")

plot(rank(facjjaaeADJUST)[order(rank(facjjaaeADJUST))],type="p",xaxt="n")
axis(1,labels=metnames[order(rank(facjjaaeADJUST))],at=1:44,cex.axis=.4)
lines(rank(infspec)[order(rank(facjjaaeADJUST))],col="blue",type="b")

plot(rank(facjjaaeRELADJUST)[order(rank(facjjaaeRELADJUST))],type="p",xaxt="n")
axis(1,labels=metnames[order(rank(facjjaaeRELADJUST))],at=1:44,cex.axis=.4)
lines(rank(infspec)[order(rank(facjjaaeRELADJUST))],col="blue",type="b")

plot((facjjaaeREL)[order((facjjaaeREL))],type="p",xaxt="n")
axis(1,labels=metnames[order((facjjaaeREL))],at=1:44,cex.axis=.4)
lines((infspec)[order((facjjaaeREL))],col="blue",type="b")

plot((facjjaae)[order((facjjaae))],type="p",xaxt="n")
axis(1,labels=metnames[order((facjjaae))],at=1:44,cex.axis=.4)
lines((infspec)[order((facjjaae))],col="blue",type="b")

plot((facjjaaeADJUST)[order((facjjaaeADJUST))],type="p",xaxt="n")
axis(1,labels=metnames[order((facjjaaeADJUST))],at=1:44,cex.axis=.4)
lines((infspec)[order((facjjaaeADJUST))],col="blue",type="b")

plot((facjjaaeRELADJUST)[order((facjjaaeRELADJUST))],type="p",xaxt="n")
axis(1,labels=metnames[order((facjjaaeRELADJUST))],at=1:44,cex.axis=.4)
lines((infspec)[order((facjjaaeRELADJUST))],col="blue",type="b")
...
#plot(rank(facjjaaeREL)[order(rank(infspec))],xaxt="n",col="blue",type="b",lty=3,pch=15,
#  xlab="Species (sorted by crossvalidation-based influence measure)",
#  ylab="Rank of influence measure")
#axis(1,labels=metnames[order(rank(infspec))],at=1:44,cex.axis=.8,las=2)
#lines(rank(infspec)[order(rank(infspec))],type="b")
#legend(30,10, c("Crossvalidation-based influence","DFFITS-based influence"), 
#   lty=c(1,3), col=c("black","blue"), pch=c(1,15))


### PLOT FOR FIGURE 1 IN INFLUENCE PAPER!!! ###
pdf("InfluenceComparison.pdf",width=10,height=7)
parmarold <- par("mar")
par(mar=c(5,4,1,1))
plot(rank(infspec)[order(rank(facjjaaeREL))],xaxt="n",col="black",type="b",lty=3,pch=15,
  xlab="Species (sorted by crossvalidation-based influence measure)",
  ylab="Rank of influence measure")
axis(1,labels=metnames[order(rank(facjjaaeREL))],at=1:44,cex.axis=.8,las=2)
lines(rank(facjjaaeREL)[order(rank(facjjaaeREL))],type="b")
legend(30,10, c("Crossvalidation-based influence",expression(paste("DFFITS-based influence (", tilde(D)[j], ")") )), 
   lty=c(1,3), col=c("black","black"), pch=c(1,15))
#legend(30,10, c("Crossvalidation-based influence",expression(tilde(D)[k]) ), 
#   lty=c(1,3), col=c("black","blue"), pch=c(1,15))
#lines(rank(diag(Hmat))[order(rank(facjjaaeREL))],type="b",col="red")
par(mar=parmarold)
dev.off()

hist( rank(facjjaaeREL) - rank(infspec) ,breaks=seq(-20,20,1))
boxplot( rank(facjjaaeREL) - rank(infspec) )
summary( rank(facjjaaeREL) - rank(infspec) )
difranks <- rank(facjjaaeREL) - rank(infspec)
#difranks <- rank(1-facjjcorr) - rank(infspec)
mean( difranks >= -10 & difranks <= 10)  #75%

aveunc <- apply(slunc2,2,mean)
W <- diag(aveunc)

Fprime <- as.matrix(lammetall)
Fprime2 <- solve(sqrt(W)) %*% Fprime

Hmat <- Fprime %*% solve(t(Fprime) %*% Fprime) %*% t(Fprime)
Hmat2 <- Fprime2 %*% solve(t(Fprime2) %*% Fprime2) %*% t(Fprime2)

#plot(rank(facjjaaeREL)[order(apply(slunc2,2,mean))],type="b",xaxt="n")
#axis(1,labels=metnames[order(apply(slunc2,2,mean))],at=1:44,cex.axis=.4)
#lines(rank(infspec)[order(apply(slunc2,2,mean))],col="blue",type="b")
#order(apply(slunc2,2,mean))
...
for (ii in 1:7)
{
  plot(facmetall[,ii],type="l",col="blue",lwd=3)
  lines(facjjarray[,ii,specnum],col="red")
...
junk <- bestcormatcors(facjjarray[,,44],facmetall)
junk

bestaaeRELmat <- function(x,y)
{
  mat <- getaaeREL(x,y)
  perms <- permn(1:ncol(mat))
  meancor <- rep(NA,length(perms))
...
getaaeREL <- function(x,y)
{
  temp <- matrix(NA,ncol(x),ncol(y))
  for (i in 1:ncol(x)) for (j in 1:ncol(y)) temp[i,j] <- mean( abs(x[,i] - y[,j]) ) / mean(y[,j])
  temp
}
...
# signal to noise
 t(t(apply(concs/uncs,2,mean)))
cbind(names(apply(concs/uncs,2,mean)), apply(concs/uncs,2,mean) )

write( t( cbind( names(apply(concs/uncs,2,mean)), apply(concs/uncs,2,mean) )), file="signal2noise.txt", ncol=2)
