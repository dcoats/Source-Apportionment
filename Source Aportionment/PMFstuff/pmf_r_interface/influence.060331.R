...
runpmfplain("default_nogkey.ini","morgc.txt","morgu.txt",mypaste("testfac",i,".txt"),
   mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0,i,nrow(morgc),ncol(morgc))    

lam<-read.table(mypaste("testlam",i,".txt"))
fac<-read.table(mypaste("testfac",i,".txt"))
lamUNWT <- lam
facUNWT <- fac
lamplots2(lamUNWT,"morg",names=mynames) 
...
runpmfplain("default_nogkey.ini","morgmetc.txt","morgmetu.txt",mypaste("testfac",i,".txt"),
   mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0,i,nrow(morgc[,1:44]),ncol(morgc[,1:44]))    
lammet<-read.table(mypaste("testlam",i,".txt"))
facmet<-read.table(mypaste("testfac",i,".txt"))
lamplots2(lammet,"morg",names=mynames) 
#lamplots2(lams7,"morg",names=mynames) 
...
lamorg<-read.table(mypaste("testlam",i,".txt"))
facorg<-read.table(mypaste("testfac",i,".txt"))
#lamplots2(lamorg,"morg",names=mynames) 
...
Ktemp[1,1] <- round(sum(tempold[grp2])/sum(tempold[grp1]),3)
...
uniquify <- function(x,grp1,grp2)
{
  ...
  for (i in 1:length(grp1)) G1[grp1[i]] <- T
  for (i in 1:length(grp2)) G2[grp2[i]] <- T
  g1 <- x[G1 & !G2] 
  g2 <- x[G2 & !G1]
  s <- x[G1 & G2]
  return(g1,g2,s)
...
for (jj in 13:15)
{
  ...
  runpmfplain("default_nogkey.ini","concs.txt", "newunc.txt" ,mypaste("testfac",numfacs,".txt"),
     ...
     mypaste("testq",numfacs,".txt"),fpeak= 0,i,nrow(concs),ncol(concs))    
  ...
  temp <-  apply(abs(dffitvals),2,median)
  dfplot(dffitvals)
  title(paste("iteration",jj,"-- \n ratio is",round(sum(temp[grp2])/sum(temp[grp1]),3),
              "(K=",K,")"))
  Ktemp[1+jj,1] <- round(sum(temp[grp2])/sum(temp[grp1]),3)
  ...
  for (jjj in 1:finetunereps)
  {
    ...
    title(paste("iteration",jj,": fine-tune \#:",jjj,"-- \n ratio is",round(sum(temp[grp2])/sum(temp[grp1]),3),
              "(K=",K,")"))
    Ktemp[1+jj,1+jjj] <- round(sum(temp[grp2])/sum(temp[grp1]),3)
  }

  Ktemp
  temp <-  apply(abs(dffitvals),2,median)
  tempunc <- matrix(NA,nrow(uncs),ncol(uncs))
  if (sum(Ktemp[jj:(1+jj),1] > K)==1) wtadjpower <- wtadjpower/2
  if (jj > 1 & abs(Ktemp[1+jj,1]-K)/K > .5) if (sum(Ktemp[jj:(1+jj),1] > K)==0 | sum(Ktemp[jj:(1+jj),1] > 1)==3) wtadjpower <- wtadjpower*1.5
...
for (i in 2:(totreps+1)) for (j in 1:(i-1)) faccomps[i,j] <- bestcormat(facarray[,,i],facarray[,,j])

round(faccomps,3)

#faccompsmse <- matrix(NA,totreps+1,totreps+1)
#for (i in 2:(totreps+1)) for (j in 1:(i-1)) faccompsmse[i,j] <- bestmsemat(facarray[,,i],facarray[,,j])
#round(faccompsmse,3)
#NO

lamcomps <- matrix(NA,totreps+1,totreps+1)
for (i in 2:(totreps+1)) for (j in 1:(i-1)) lamcomps[i,j] <- bestrevcormat(lamarray[,,i],lamarray[,,j])
round(lamcomps,3)

#lamcompsdumb <- matrix(NA,totreps+1,totreps+1)
#for (i in 2:(totreps+1)) for (j in 1:(i-1)) lamcompsdumb[i,j] <- bestcormat(lamarray[,,i],lamarray[,,j])
