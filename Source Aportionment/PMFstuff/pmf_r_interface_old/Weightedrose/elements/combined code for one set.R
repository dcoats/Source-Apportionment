plots<-function(style,wind,species,nsector=16,upper=.25,main='sample',
	...
	if (radius <= 1.5){
		label<-signif(radius/2,1)
	} else {label<-signif(radius/2)}
...
b<-0
smeas<-0
pdf("F:/EPA/Weightedrose/elements/testagain.pdf",paper="letter")
par(mar=c(.5,1,3.5,1),mfrow=c(3,3))
for (i in 1:length(elements)){
	for (st in c('wtrose','hopke')){
		set<-c('wtrose','hopke','simple')
		r<-c(3,.6,.2)
		plots(st,winddir,dat[,elements[i]+2],s,p,
			specnames[elements[i]],measurename=m,radius=r[set==st])
	}
	b<-b+1
	compare[b,1]<-m
	compare[b,2]<-p
	...
	meascompare<-c(wmeas,hmeas,smeas)
	best<-c("W","H","S")
	compare[b,5]<-best[meascompare==max(meascompare)]
}
dev.off()
compare<-as.data.frame(compare)
compare
write.csv(dat,"F:/EPA/Weightedrose/elements/elements.csv",col.names=T)
dat[67.5<dat[,2] & dat[,2]<82.5,c(1,39+2)]
leadset<-cbind(winddir,Pb)
dat[Pb==max(Pb),]
leadset[337.5<winddir & winddir<352.5,]
nonmaxleadset<-leadset[-80,]
nonmaxleadset[337.5<nonmaxleadset[,1] & nonmaxleadset[,1]<352.5,]
plot(Pb[67.5<winddir & winddir<82.5])
leadset[Pb==max(nonmaxleadset[,2])]
dat[337.5<winddir & winddir<352.5,c(1,2,39+2)]
dat[67.5<winddir & winddir<82.5,c(1,2,39+2)]
dat[,c(1,2,39+2)]