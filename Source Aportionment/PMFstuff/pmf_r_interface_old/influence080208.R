...
plot(Ks,corrswithfacUNWT[-c(1,10)],ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,corrswithfacmet[-c(1,10)],col="blue",lty="dashed",lwd=2)
lines(Ks,corrswithfacorg[-c(1,10)],col="red",lty="dotted",lwd=2)
axis(1,at=Ks,labels=c(Ks),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   ...
   lty=c("solid","dashed","dotted"),lwd=c(1,2,2),col=c("black","blue","red"))  #This creates the legend
#abline(h=corrswithfacmet[10],lty="dotted",lwd=.5,col="gray")
dev.off()
...
plot(Ks,expcorrswithlamUNWT,ylim=c(0,1),ylab="Similarity",
   xlab="" ,xaxt="n",log="x",type="l")
mtext("W",side=1,line=4,font=3)
lines(Ks,expcorrswithlammet,col="blue",lty="dashed",lwd=2)
lines(Ks,expcorrswithlamorg,col="red",lty="dotted",lwd=2)
axis(1,at=Ks,labels=c("MET",Ks[2:(length(Ks)-1)],"ORG"),las=2,cex=.3)
#title("Contribution similarities with UNWT (black), MET (blue), and ORG (red)") 
abline(v=0.0358,lty="dotted",lwd=.5)
#abline(v=1,lty="dotted",lwd=.5)
legend("bottomleft",inset=c(.05,.05),
   legend=c("Unmanipulated complete data","Elements data","Organics data")   ,
   lty=c("solid","dashed","dotted"),lwd=c(1,2,2),col=c("black","blue","red"))  
...
### PLOT FOR FIGURE 4 IN PAPER!!! ###
plot(rank(infspec)[order(rank(facjjaaeREL))],xaxt="n",col="blue",type="b",lty=3,pch=15,
  xlab="Species (sorted by crossvalidation-based influence measure)",
  ylab="Rank of influence measure")
