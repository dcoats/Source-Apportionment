...
lamarrayerrsMMBIG <- array(NA,dim=c(ncol(concs),numfacs,length(cvvals),reperrorvals))
facarrayerrsMMBIG <- array(NA,dim=c(nrow(concs),numfacs,length(cvvals),reperrorvals))
...
# for (cvi in 9:11)
{
  ...
  #for (reperri in 1:5)
  {
    ...
    #uncerrs <- uncs
    # uncerrs <- uncerrs*10
    concerrs <- concs * matrix( exp( rnorm(nrow(concs)*ncol(concs), -.5*log((cvvals[cvi])^2+1) , sqrt(log((cvvals[cvi])^2+1))) ) , nrow(concs),ncol(concs))
    ...
    runpmfplain("default_nogkey.ini","concerrs.txt", "lhtunc2.txt" ,mypaste("testfac",numfacs,".txt"),
     ...
     lamarrayerrsMMBIG[,,cvi,reperri] <- as.matrix(lam)
     facarrayerrsMMBIG[,,cvi,reperri] <- as.matrix(fac)
...
### ALTERNATIVE...FASTER ###
faccompsallMMBIG <- array(NA,dim=c(reperrorvals,numfacs,length(cvvals)))
lamcompsallMMBIG <- array(NA,dim=c(reperrorvals,numfacs,length(cvvals)))
faccompsallrmseMMBIG <- array(NA,dim=c(reperrorvals,numfacs,length(cvvals)))
lamcompsallexpMMBIG <- array(NA,dim=c(reperrorvals,numfacs,length(cvvals)))
...
{
  faccompsallMMBIG[i,,cvi] <- apply( cor(facarrayerrsMMBIG[,,cvi,i],facORIG) , 2, max)
  lamcompsallMMBIG[i,,cvi] <- apply( revcormat(lamarrayerrsMMBIG[,,cvi,i],lamORIG) , 2, max)
  faccompsallrmseMMBIG[i,,cvi] <- apply( getaae(facarrayerrsMMBIG[,,cvi,i],facORIG) , 2, min) ## now does aae
  lamcompsallexpMMBIG[i,,cvi] <- apply( expcormat(lamarrayerrsMMBIG[,,cvi,i],lamORIG,
                                           facarrayerrsMMBIG[,,cvi,i],facORIG) , 2, max)
}
}
