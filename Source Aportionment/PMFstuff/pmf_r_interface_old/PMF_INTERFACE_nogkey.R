...
mynames <- names <- c("Na","Mg","Al","Si","Ph",
		"S","Cl","K","Ca","Ti",
		"V","Cr","Mn","Fe","Co",
		"Ni","Cu","Zn","Ga","As",
		"Se","Br","Rb","Sr","Y",
...
sldat2 <- sldat
slunc2 <- slunc
sldat2[sldat2== -999.9] <- slunc2[sldat2== -999.9] <- NA
sldat2 <- na.omit(sldat2)
slunc2 <- na.omit(slunc2)
datesnew <- getdates()
newdates <- datesnew[apply(sldat==-999.9,1,sum)==0]
dimnames(sldat2)[[2]] <- names





runpmfplain("default_nogkey.ini",sldat2,slunc2,mypaste("testfac",i,".txt"),mypaste("testlam",i,".txt"),
   mypaste("testq",i,".txt"),fpeak= 0.1,i,nrow(sldat2),ncol(sldat2))    
   #   if no gkeying is used, change "keydat" to "sldat"
...
weekendplots2<-function(i,name="gkeyw",datesnew)
{
  ####weekend plots
  library(survival)
  #datesnew <- newdates
  datesnew<-as.character(datesnew)
  datesnew<-as.date(datesnew,order='ymd')
  days<-date.mdy(datesnew,weekday=TRUE)$weekday
...
weekendplots3<-function(i,datesnew,facname=fac,name="gkeyw")
{
  ####weekend plots
  library(survival)
  datesnew<-as.character(datesnew)
  ...
  for(ii in 1:i)
  {
    for(iii in c(1:7))
    {
      daycounts[ii,iii,1]<-mean(facname[,ii][days==iii])
    }
    jpeg(filename=mypaste(name,i,ii,".jpg") ,width=400, height=250)
    plot(daycounts[ii,,],type="l",xaxt="n",ylab="",xlab=mypaste("Mean by days of week ",i,", ",ii))
    axis(1,at=c(1:7),labels=weekdays)
...
facplots2<-function(fac,name="NOgkeyf",datevec){
#start on a logical date near the beginning of the data
dates<-datevec
for(iii in 1:ncol(fac)){
jpeg(filename=mypaste(name,i,iii,".jpg") ,width=700, height=250)
...
plot(dates,fac[,iii],type="l",col="blue",xlab="",xaxt="n",ylab=mypaste(iii," of ",i," factors"))
par(cex=.7)
axis.Date(1,at=seq(datevec[1],datevec[length(datevec)],"months"),
format="%b-%y",las=2)
dev.off()}
}
...
lamplots2<-function(lam,name="gkey",names=mynames,facnames){
for(iii in 1:ncol(lam)){
	...
	par(cex=.7)
	plot(lam[,iii],ylim=c(0,max(lam[,iii])+.05),ylab="",
	  xlab="",#mypaste("Source ",i,", ",iii),
        xaxt="n",las=2,type="n",tcl= -.2)
      par(cex=1.25)
      title(ylab=facnames[iii])
	par(cex=.7)
	axis(1,at=1:length(lam[,iii]),label=names,las=2,tcl=-.2)
	###  makes a box around significant values
	par(cex=1)	
	for(ii in 1:nrow(lam)){
	if(lam[ii,iii]>=.01){
...
lamplots2ps<-function(lam,name="gkey",names=mynames,facnames){
for(iii in 1:ncol(lam)){
	...
	for(ii in 1:nrow(lam)){
	if(lam[ii,iii]>=.01){
	rect(ii-.5,par("usr")[3]+.01,ii+.5,par("usr")[4]-.01,col="grey90",border="transparent")
	}}
	points(lam[,iii],bty="o") #redraws the points on top of the shaded boxes
	#end of box maxing
	#print(par("usr"))
...
lamplots2ps(lam9gkeynostart,"gkey",names=mynames,facnames) 




lamplotslog<-function(lam,name="gkey",names=mynames,facnames=NULL){
for(iii in 1:ncol(lam)){
	jpeg(filename=mypaste(name,i,iii,".jpg") ,width=700, height=250)
	par(cex=1)
	plot(lam[,iii],ylim=c(0.00001,1),log="y",ylab="",
	   xlab=mypaste("Source ",i,", ",iii),xaxt="n",yaxt="n")
	...
	for(ii in 1:nrow(lam)){
	if(lam[ii,iii]>=.01){
	rect(ii-.5,0.00001,ii+.5,1,col="grey90",border="transparent")
	}}
	points(lam[,iii],bty="o") #redraws the points on top of the shaded boxes
	#end of box maxing
	#print(par("usr"))
...
#lam.old <- lam
#lam <- lam9gkeynostart[,reorderfacs]
#name <- "gkey"
#facnames2 <- facnames[reorderfacs]

lamplotslogps<-function(lam,name="gkey",names=mynames,facnames=NULL){
for(iii in 1:ncol(lam))
   {
	pdf(file=mypaste(name,i,iii,".pdf") ,width=8.5, height=1.5)
            par(mar=c(3,6.5,1,1))
...
#	plot(lam[,iii],ylim=c(0.00001,1),log="y",ylab="",
#	   xlab=mypaste("Source ",i,", ",iii),xaxt="n",yaxt="n")
	barplot(  apply(cbind(0,(log(lam[,iii])-log(.0001))/(-log(.0001))),1,max) ,
         ylim=c(0.00001,1),ylab="",
	   xlab=mypaste("Source ",i,", ",iii),yaxt="n",names.arg=names,las=2)
#	barplot((lam[,iii]),add=T)
	axis(2,at=c(0,.25,.5,.75,1),labels=c(".0001",".001",".01",".1","1"),las=2,cex=.3)
#	axis(4,at=c(0,.25,.5,.75,1),labels=c(".0001",".001",".01",".1","1"),las=2,cex=.3)
      par(cex=1)
      title(ylab=facnames[iii])
      par(cex=.7)
	abline(h=0)
