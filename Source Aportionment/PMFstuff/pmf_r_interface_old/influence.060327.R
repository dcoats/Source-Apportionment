...
par(mfrow=c(3,2))
dfplot(dffitvals)
  title(paste("iteration 0-- ratio is",round(sum(tempold[grp2])/sum(tempold[grp1]),3),
              "(K=",K,")"))
...
for (jj in 1:6)
{
  ...
  runpmfplain("default_nogkey.ini","concs.txt", "newunc.txt" ,mypaste("testfac",i,".txt"),
     mypaste("testlam",i,".txt"),
     mypaste("testq",i,".txt"),fpeak= 0,i,nrow(concs),ncol(concs))    
  lam<-read.table(mypaste("testlam",i,".txt"))
  Lambda <- lam
  if (ncol(Lambda) >= 1) l1 <- as.double(Lambda[,1])
  if (ncol(Lambda) >= 2) l2 <- as.double(Lambda[,2])
...
## START HERE ##
  ...
  weekendplots3(i,newdates,facname=facjj,"morgw")
  system(mypaste("multipanel_morg",i,".bat"),invisible=TRUE) #see License in this directory for the imagemagick license
}
## END OF WEIGHT ADJUSTMENT CODE
