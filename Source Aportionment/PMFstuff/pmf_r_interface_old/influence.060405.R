...
#for (jj in 4:totreps)
{
  ...
  #if (jj > 1 & abs(Ktemp[1+jj,1]-K)/K > .3) {if (sum(Ktemp[jj:(1+jj),1] > K)==0 | sum(Ktemp[jj:(1+jj),1] > K)==2) wtadjpower <- wtadjpower*1.5}
  #else 
  if (jj > 3 & abs(Ktemp[1+jj,1]-K)/K > .02) {if (sum(Ktemp[(jj-2):(1+jj),1] > K)==0 | sum(Ktemp[(jj-2):(1+jj),1] > K)==4) wtadjpower <- wtadjpower*1.25}
